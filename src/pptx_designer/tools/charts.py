...
def bar_chart(slide, left, top, data, max_width=5.0, bar_height=0.3, C=None, typo=None, spacing=None, grouped=True):
    ...
    if grouped:
        ...
        for i, (label, pct, val) in enumerate(data):
            ...
            bg = _add_shape(
                gs, MSO_SHAPE.ROUNDED_RECTANGLE, Inches(left), Inches(y), Inches(max_width), Inches(bar_height)
            )
            bg.fill.solid()
            ...
            if bar_w > 0:
                bar = _add_shape(
                    gs, MSO_SHAPE.ROUNDED_RECTANGLE, Inches(left), Inches(y), Inches(bar_w), Inches(bar_height)
                )
                bar.fill.solid()
                bar.fill.fore_color.rgb = _rgb(bar_colors[i % len(bar_colors)])
                bar.line.fill.background()

            lbl_box = gs.add_textbox(Inches(left - 0.9), Inches(y - 0.03), Inches(0.85), Inches(bar_height))
            p = lbl_box.text_frame.paragraphs[0]
            _set_run(
                p, label, font_size=t.caption, color=C.get("text_body", "#333333"), font_name=C.get("font_body"), C=C
            )
            p.alignment = PP_ALIGN.RIGHT

            val_box = gs.add_textbox(Inches(left + max_width + 0.08), Inches(y - 0.03), Inches(0.6), Inches(bar_height))
        ...
        for i, (label, pct, val) in enumerate(data):
            ...
            bar_w = max_width * pct
            if bar_w > 0:
                rrect(slide, left, y, bar_w, bar_height, bar_colors[i % len(bar_colors)], C=C)
            text(
                slide,
                left - 0.9,
                y - 0.03,
...
def comparison_bars(slide, left, top, metrics, max_width=4.0, C=None, typo=None, spacing=None, grouped=True):
    ...
    if grouped:
        ...
        for idx, (label, v_old, v_new, pct_old, pct_new) in enumerate(metrics):
            ...
            lbl_box = gs.add_textbox(Inches(left - 1.1), Inches(y - 0.02), Inches(1.0), Inches(0.2))
            p = lbl_box.text_frame.paragraphs[0]
            _set_run(
                p,
                label,
            ...
            bg1 = _add_shape(gs, MSO_SHAPE.ROUNDED_RECTANGLE, Inches(left), Inches(y), Inches(max_width), Inches(0.18))
            bg1.fill.solid()
            ...
            if bar_old > 0:
                b1 = _add_shape(gs, MSO_SHAPE.ROUNDED_RECTANGLE, Inches(left), Inches(y), Inches(bar_old), Inches(0.18))
                b1.fill.solid()
                b1.fill.fore_color.rgb = _rgb(C.get("muted", "#81C784"))
                b1.line.fill.background()

            bg2 = _add_shape(
                gs, MSO_SHAPE.ROUNDED_RECTANGLE, Inches(left), Inches(y + 0.22), Inches(max_width), Inches(0.18)
            )
            bg2.fill.solid()
            ...
            if bar_new > 0:
                b2 = _add_shape(
                    gs, MSO_SHAPE.ROUNDED_RECTANGLE, Inches(left), Inches(y + 0.22), Inches(bar_new), Inches(0.18)
                )
                b2.fill.solid()
                b2.fill.fore_color.rgb = _rgb(C.get("primary", "#1B5E20"))
                b2.line.fill.background()

            old_box = gs.add_textbox(Inches(left + max_width + 0.1), Inches(y - 0.03), Inches(0.8), Inches(0.2))
            p2 = old_box.text_frame.paragraphs[0]
            _set_run(
                p2, v_old, font_size=t.micro, color=C.get("text_muted", "#666666"), font_name=C.get("font_body"), C=C
            )

            new_box = gs.add_textbox(Inches(left + max_width + 0.1), Inches(y + 0.19), Inches(0.8), Inches(0.2))
...
def donut_chart(slide, cx, cy, radius, inner_radius, sectors, C=None, typo=None, grouped=True, native=True):
    ...
    if grouped:
        ...
        inner = _add_shape(
            gs,
            MSO_SHAPE.OVAL,
            Inches(cx - inner_radius),
            Inches(cy - inner_radius),
            Inches(inner_radius * 2),
            Inches(inner_radius * 2),
        )
        inner.fill.solid()
        ...
        center_box = gs.add_textbox(Inches(cx - 0.5), Inches(cy - 0.2), Inches(1.0), Inches(0.4))
        p = center_box.text_frame.paragraphs[0]
        _set_run(
            p,
            "100%",
            font_size=t.h2,
            color=C.get("primary", "#1B5E20"),
            bold=True,
        ...
        for name, pct_str, clr in sectors:
            dot = _add_shape(gs, MSO_SHAPE.ROUNDED_RECTANGLE, Inches(lx), Inches(ly), Inches(0.2), Inches(0.2))
            dot.fill.solid()
            dot.fill.fore_color.rgb = _rgb(clr)
            dot.line.fill.background()

            lbl = gs.add_textbox(Inches(lx + 0.3), Inches(ly - 0.02), Inches(1.5), Inches(0.25))
