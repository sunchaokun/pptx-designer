...
def kpi_card(
    ...
    # Background
    bg = rrect(slide, left, top, width, height, C.get("card", "#FFFFFF"), line=C.get("border", "#E5E5E5"))
...
def highlight_cards(slide, left, top, cards, total_width=12.0, C=None, typo=None, spacing=None, grouped=True):
    ...
    for i, (title, desc, accent) in enumerate(cards):
        ...
        # Card background
        bg = rrect(slide, x, top, card_width, 1.8, C.get("card", "#FFFFFF"), line=C.get("border", "#E5E5E5"))
        shapes.append(bg)
        # Accent bar
...
def code_block(slide, left, top, width, height, lines, language="python", C=None, typo=None, grouped=True):
...
def _add_shape(slide, left, top, width, height, txt, font_size=12, color="#000000", bold=False, font_name=None, C=None):
    """Add a text shape."""
