...
def set_widescreen(prs):
    ...
    sldSz = prs._element.find("{http://schemas.openxmlformats.org/presentationml/2006/main}sldSz")
...
def clean_save(prs, path):
    ...
    for name, data in items.items():
        if name == "ppt/_rels/presentation.xml.rels":
            ...
            to_remove = [r for r in root if "printerSettings" in r.get("Target", "")]
            if to_remove:
                for r in to_remove:
                    root.remove(r)
                rid_map = {}
                for r in root:
                    ...
                    if old.startswith("rId"):
                        rid_map[old] = f"rId{len(rid_map) + 1}"
                for r in root:
                    old = r.get("Id", "")
                    if old in rid_map:
    ...
    if rid_map:
        for name in list(items.keys()):
            if name == "ppt/presentation.xml":
                ...
                items[name] = _et.tostring(root, xml_declaration=True, encoding="UTF-8", standalone=True)
...
def page_number(slide, current, total, style="simple", C=None, typo=None):
    ...
    if style == "simple":
        ...
        bar_fill = rect(slide, 2.5, 7.1, 8.0 * progress, 0.08, C.get("accent", "#2E6504"))
...
def copy_decorations(slide, template_slide, skip_long_text=True, skip_image=True):
    for shape in template_slide.shapes:
        if skip_image and shape.shape_type == 13:
            continue
        if skip_long_text and shape.has_text_frame and len(shape.text_frame.text) > 50:
            continue
