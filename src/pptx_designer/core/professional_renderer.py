...
def render_professional_page(
    ...
    from pptx_designer.tools.shapes import rect, oval, rrect
    ...
    # ── Page number ───────────────────────────────────────────────
    if page_index > 0:
        text(slide, 12.0, 7.0, 1.0, 0.3, f"{page_index + 1}/{total_pages}",
             font_size=10, color=text_muted, align="right")


def _render_hero(slide: Any, title: str, subtitle: str, primary: str, accent: str, text_dark: str, text_body: str) -> None:
    ...
    # Title
    text(slide, 1.0, 3.0, 8.0, 1.2, title,
         font_size=48, bold=True, color=text_dark)

    # Subtitle
    if subtitle:
        text(slide, 1.0, 4.3, 8.0, 0.6, subtitle,
             font_size=20, color=text_body)


def _render_problem(slide: Any, title: str, bullets: list[str], primary: str, accent: str, text_dark: str, text_body: str, bg_card: str) -> None:
    """Render problem slide with visual impact."""
    from pptx_designer.tools.shapes import rect, oval, rrect
    from pptx_designer.tools.text import text

    # Section label
    text(slide, 1.0, 0.5, 3.0, 0.3, "PROBLEM",
         font_size=12, bold=True, color=accent)

    # Title
    text(slide, 1.0, 0.9, 11.0, 0.8, title,
         font_size=36, bold=True, color=text_dark)
    ...
    if bullets:
        ...
        for j, bullet in enumerate(bullets[:3]):
            ...
            text(slide, x + card_width/2 - 0.3, 2.7, 0.6, 0.6, str(j + 1),
                 font_size=24, bold=True, color="#FFFFFF", align="center")
            # Problem text
            text(slide, x + 0.2, 3.6, card_width - 0.4, 1.8, bullet,
                 font_size=14, color=text_body, align="center")


def _render_solution(slide: Any, title: str, bullets: list[str], primary: str, accent: str, text_dark: str, text_body: str, bg_card: str) -> None:
    """Render solution slide."""
    from pptx_designer.tools.shapes import rect, oval, rrect
    from pptx_designer.tools.text import text

    # Section label
    text(slide, 1.0, 0.5, 3.0, 0.3, "SOLUTION",
         font_size=12, bold=True, color="#22C55E")
    ...
    if bullets:
        ...
        for j, bullet in enumerate(bullets[:4]):
            ...
            text(slide, 1.05, y + 0.05, 0.4, 0.4, "✓",
                 font_size=18, bold=True, color="#FFFFFF", align="center")
            # Text
            text(slide, 1.8, y + 0.05, 10.0, 0.4, bullet,
...
def _render_features(slide: Any, title: str, bullets: list[str], primary: str, accent: str, text_dark: str, text_body: str, bg_card: str) -> None:
    """Render features with gradient cards."""
    from pptx_designer.tools.shapes import rect, oval, rrect
    from pptx_designer.tools.text import text

    # Section label
    text(slide, 1.0, 0.5, 3.0, 0.3, "FEATURES",
    ...
    if bullets:
        ...
        for j, (bullet, color) in enumerate(zip(cards, colors)):
            x = 1.0 + j * (card_width + 0.4)
            ...
            text(slide, x + card_width/2 - 0.4, 2.9, 0.8, 0.8, "★",
                 font_size=32, color="#FFFFFF", align="center")
            # Feature text
            text(slide, x + 0.3, 4.1, card_width - 0.6, 1.5, bullet,
                 font_size=14, color=text_body, align="center")


def _render_data(slide: Any, title: str, bullets: list[str], primary: str, accent: str, text_dark: str, text_body: str, bg_card: str) -> None:
    """Render data/metrics with KPI cards."""
    from pptx_designer.tools.shapes import rect, oval, rrect
    from pptx_designer.tools.text import text

    # Section label
    text(slide, 1.0, 0.5, 3.0, 0.3, "METRICS",
         font_size=12, bold=True, color="#F59E0B")
    ...
    if bullets:
        ...
        if len(kpi_data) <= 4:
            ...
            for j, (value, label) in enumerate(kpi_data):
                ...
                # Value
                text(slide, x, 3.0, card_width, 0.8, value,
                     font_size=36, bold=True, color=color, align="center")
                # Label
                text(slide, x, 4.0, card_width, 0.4, label,
                     font_size=12, color=text_body, align="center")


def _render_code(slide: Any, title: str, bullets: list[str], primary: str, accent: str, text_dark: str, text_body: str, bg_card: str) -> None:
    """Render code slide."""
    from pptx_designer.tools.shapes import rect, rrect
    from pptx_designer.tools.text import text

    # Section label
    text(slide, 1.0, 0.5, 3.0, 0.3, "CODE",
         font_size=12, bold=True, color="#22C55E")
    ...
    text(slide, 1.25, 2.22, 0.9, 0.25, "Python",
         font_size=10, bold=True, color="#FFFFFF", align="center")

    # Code lines
    if bullets:
        y = 2.7
        for line in bullets[:8]:
            text(slide, 1.5, y, 10.0, 0.25, line,
                 font_size=12, color="#E6EDF3", font_name="Consolas")
            y += 0.35


def _render_content(slide: Any, title: str, bullets: list[str], primary: str, accent: str, text_dark: str, text_body: str, bg_card: str) -> None:
    """Render generic content slide."""
    from pptx_designer.tools.shapes import rect, rrect
    from pptx_designer.tools.text import text

    # Title
    text(slide, 1.0, 0.5, 11.0, 0.8, title,
         font_size=36, bold=True, color=text_dark)
    ...
    if bullets:
        ...
        for j, bullet in enumerate(bullets[:5]):
            ...
            # Text
            text(slide, 1.8, y + 0.15, 10.0, 0.5, bullet,
                 font_size=15, color=text_body)
            y += 1.0
